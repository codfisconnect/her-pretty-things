...
export function adminLogin(request: Request, response: Response) {
  ...
  if (typeof body !== "object" || body === null)
    throw new HttpError(400, "Login payload is required.");
...
export function adminSession(request: Request, response: Response) {
  response.json({
    success: true,
    data: { authenticated: isAdminSessionValid(request) },
  });
}
...
export async function orders(_request: Request, response: Response) {
  response.json({ success: true, data: await listOrders() });
}

export async function orderDetails(request: Request, response: Response) {
  response.json({
    success: true,
    data: await getOrder(readString(request.params.orderId, "orderId")),
  });
}
...
export async function orderStatus(request: Request, response: Response) {
  const body = request.body as unknown;
  if (typeof body !== "object" || body === null)
    throw new HttpError(400, "Status payload is required.");
  response.json({
    success: true,
    data: await updateOrderStatus(
      readString(request.params.orderId, "orderId"),
      readString((body as Record<string, unknown>).status, "status"),
    ),
  });
}
...
export async function adminProductDetails(
  request: Request,
  response: Response,
) {
  const productId = readString(request.params.productId, "productId");
...
export async function deleteAdminProductImage(
  ...
  const productId = readString(request.params.productId, "productId");
  const imageId = readString(request.params.imageId, "imageId");
